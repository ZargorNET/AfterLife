...
public class Passwordreset extends PageRequest {
	...
	public DefaultFullHttpResponse onRequest(ChannelHandlerContext ctx, FullHttpReq req, Module associatedModule) throws Exception {
		...
		net.zargor.afterlife.server.requests.modules.Passwordreset.PasswordresetCode code;
		synchronized (((net.zargor.afterlife.server.requests.modules.Passwordreset) associatedModule).getCodes()) {
			code = ((net.zargor.afterlife.server.requests.modules.Passwordreset) associatedModule).getCodes().stream().filter(passwordresetCode -> passwordresetCode.getCode().equals(req.getGetParameters().get("code"))).findFirst().orElse(null);
		}
